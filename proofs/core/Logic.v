...
Definition satisfies (pd: pi_dom)
  ...
  formula_rep gamma_valid pd pf vt vv f f_typed.
...
Theorem closed_satisfies_equiv (pd: pi_dom)  
(pf: pi_funpred gamma_valid pd)
(pf_full: full_interp gamma_valid pd pf) (f: formula)
(Hc: closed gamma f):
reflect (satisfies pd pf pf_full f (f_ty Hc))
  (formula_rep gamma_valid pd pf triv_val_typevar (triv_val_vars _ _) 
    f (f_ty Hc)).
Proof.
...
formula_rep gamma_valid pd pf triv_val_typevar
...
Corollary closed_satisfies_dec (pd: pi_dom)
(pf: pi_funpred gamma_valid pd)
(pf_full: full_interp gamma_valid pd pf) (f: formula)
(Hc: closed gamma f):
{ satisfies pd pf pf_full f (f_ty Hc) } +
...
Theorem semantic_lem (pd: pi_dom)
(pf: pi_funpred gamma_valid pd)
(pf_full: full_interp gamma_valid pd pf) (f: formula) 
(Hc: closed gamma f):
satisfies pd pf pf_full f (f_ty Hc) \/
...
Proof.
  ...
  rewrite fmla_rep_irrel with(Hval1:= (typed_not_inv (f_ty (closed_not Hc))))
    (Hval2:=f_ty Hc).
  destruct (formula_rep gamma_valid pd pf triv_val_typevar 
