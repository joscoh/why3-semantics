...
Notation term_rep := (term_rep gamma_valid pd pf).
Notation formula_rep := (formula_rep gamma_valid pd pf).
...
Lemma true_impl (vv: val_vars pd vt) (f: formula) (Hval1: formula_typed gamma f)
  ...
  formula_rep vt vv f Hval1 =
  formula_rep vt vv (Fbinop Timplies Ftrue f) Hval2.
Proof.
...
Lemma and_impl (vv: val_vars pd vt) 
  ...
  formula_rep vt vv (Fbinop Timplies (Fbinop Tand f1 f2) f3) Hval1 =
  formula_rep vt vv (Fbinop Timplies f1 (Fbinop Timplies f2 f3)) Hval2.
Proof.
  simpl_rep_full.
  rewrite implb_curry.
...
Lemma fmla_rewrite vv (f1 f2: formula) (Heq: f1 = f2)
  ...
  formula_rep vt vv f1 Hval1 = formula_rep vt vv f2 Hval2.
Proof.
...
Lemma distr_impl_let_forall (vv: val_vars pd vt)  
  ...
  (Hq: forall x, ~ (In x q /\ aset_mem x (fmla_fv f1)))
  (Hl: forall x, ~ (In x l /\ aset_mem (fst x) (fmla_fv f1))) :
  formula_rep vt vv
...
formula_rep vt vv (fforalls q (Flet t x f)) Hval1 =
formula_rep vt vv (Flet t x (fforalls q f)) Hval2.
Proof.
    ...
    ((fix build_indpred (l: list (list formula)) 
      ...
      | fs :: ftl => fun Hall =>
        iter_and (map is_true (dep_map (@formula_rep _ gamma_valid pd
         (interp_with_Ps gamma_valid pd pf _ Ps) (mk_vt (s_params p) srts) (mk_vv _))
            fs (Forall_inv Hall))) /\
...
Definition indpred_rep_single (pf: pi_funpred gamma_valid pd) 
  ...
  (forall (P: forall (srts: list sort), 
    ...
    iter_and (map is_true (dep_map (@formula_rep _ gamma_valid 
      pd (interp_with_P gamma_valid pd pf p P) (mk_vt (s_params p) srts) (mk_vv _)) 
      fs Hform)) -> P srts a).
...
Definition Forall_single {A: Type} {P: A -> Prop} {x: list A}
  (Hform: Forall P x) :
  Forall (Forall P) [x] :=
  Forall_cons _  _ _ Hform (@Forall_nil (list A) (Forall P)).
...
Proof.
  ...
  - generalize dependent (in_triv p fs). simpl.
    ...
    revert Hand.
    rewrite (interp_with_Ps_single gamma_valid pd pf p Ps); intros Hand.
  ...
  - revert H. generalize dependent (in_triv p fs); simpl.
    ...
    rewrite (interp_with_Ps_single gamma_valid pd pf p _). simpl.
...
(*One of the complications is that the [build_indpred]
  ...
  ((fix build_indpred
          ...
          (map is_true
            (dep_map
                (formula_rep gamma_valid pd
                  (interp_with_Ps gamma_valid pd pf ps Ps) vt vv)
                fs (Forall_inv Hall))) /\
        build_indpred ftl (Forall_inv_tail Hall)
    end Hl) fs Hform) <->
    ...
    (map is_true
        (dep_map
          (formula_rep gamma_valid pd
              (interp_with_Ps gamma_valid pd pf ps Ps) vt vv) f Hallf))).
Proof.
  revert Hform.
  ...
  (forall (fs : list formula) (Hform : Forall (formula_typed gamma) fs),
    In fs (map snd ps) ->
      iter_and (map is_true (dep_map
        (formula_rep gamma_valid pd
        (interp_with_Ps gamma_valid pd pf (map fst ps) Ps)  (mk_vt (s_params p) srts) (mk_vv _)) fs Hform))) ->
...
Theorem indpred_least_pred_val (pf: pi_funpred gamma_valid pd) 
  ...
  (Hsub: Forall (fun f => asubset (fmla_type_vars f) (list_to_aset (s_params p))) 
    (concat (map snd ps))) 
  ...
  (forall (fs : list formula) (Hform : Forall (formula_typed gamma) fs),
    In fs (map snd ps) ->
      iter_and (map is_true (dep_map
        (formula_rep gamma_valid pd
        (interp_with_Ps gamma_valid pd pf (map fst ps) Ps) vt vv) fs Hform))) ->
(*Then indpred_rep p fs x -> P x*)  
    indpred_rep pf ps Hform p Hinp srts a ->
    get_hlist_elt predsym_eq_dec Ps p Hinp srts a.
Proof.
  ...
  - intros.
    ...
    assert (Hinx: In x0 (s_params p)). {
      setoid_rewrite asubset_def in Hsub.
      specialize (Hsub _ H0 _ H1). simpl_set. auto.
    }
  ...
  - intros.
    ...
    exfalso. apply (aset_is_empty_mem _ _ _ Hclosed Hinx).
Qed.
...
Lemma indpred_decomp_equiv (pf: pi_funpred gamma_valid pd) 
  ...
  (Hwf: fmla_wf f) :
  formula_rep gamma_valid pd pf vt vv f Hval =
...
Proof.
  ...
  (vv : val_vars pd vt),
  formula_rep gamma_valid pd pf vt vv f Hval =
  formula_rep gamma_valid pd pf vt vv (indpred_transform f) v); auto;
  ...
  - (*On to let case*)
    ...
    apply (H3 y). 
    split_all; auto. simpl_set. left; auto.
...
(Hclosed: Forall closed_formula fs) :
  iter_and (map is_true (dep_map
    (formula_rep gamma_valid pd pf vt v1) fs Hform)) =
  iter_and (map is_true (dep_map
    (formula_rep gamma_valid pd pf vt v2) fs Hform)).
Proof.
  f_equal. f_equal.
  revert Hform.
  induction fs; simpl; auto.
...
Lemma strict_pos_impred_implies_P' (pf: pi_funpred gamma_valid pd) 
  ...
  iter_and (map is_true (dep_map
    (formula_rep gamma_valid pd 
      (interp_with_Ps gamma_valid pd pf (map fst ps) Ps) (mk_vt (s_params p) srts) vv) fs Hform))) ->
  (*Then [[f]]_(p->indpred_rep p) implies [[f]]_(p->P)*) 
  formula_rep gamma_valid pd pf (mk_vt (s_params p) srts) vv f Hvalf ->
  formula_rep gamma_valid pd  (interp_with_Ps gamma_valid pd pf (map fst ps) Ps) (mk_vt (s_params p) srts) vv f Hvalf.
Proof.
  intros Ps HandPs.
  assert (pf_eq : pf_same_constrs pf (interp_with_Ps gamma_valid pd pf (map fst ps) Ps)). { apply pf_same_constrs_refl; auto. }
  ...
  - (*Show arg lists are the same: because P cannot appear
    ...
    (term_rep gamma_valid pd pf (mk_vt (s_params p) srts) vv) Hvalf) =  
    (pred_arg_list pd (mk_vt (s_params p) srts) p0 vs ts
       (term_rep gamma_valid pd (interp_with_Ps gamma_valid pd pf (map fst ps) Ps)
          (mk_vt (s_params p) srts) vv) Hvalf)). {
      (*TODO: name*)
      apply Denotational.get_arg_list_eq.
      rewrite Forall_forall. intros.
      rewrite term_rep_irrel with(Hty2:=Hty2).
      apply tm_change_pf; simpl; auto; intros p' srts' a' Hinp'. symmetry.
      destruct (in_bool_spec predsym_eq_dec p' (map fst ps));
      [|rewrite find_apply_pred_notin; auto].
  ...
  - rewrite !bool_of_binop_impl, !simpl_all_dec.
    ...
    rewrite fmla_change_pf with(p2:=(interp_with_Ps gamma_valid pd pf (map fst ps) Ps)); auto.
    intros p' srts' a' Hinp'.
    simpl. symmetry.
    destruct (in_bool_spec predsym_eq_dec p' (map fst ps));
    [|rewrite find_apply_pred_notin; auto].
  ...
  - (*First, know that [[f1]] eq in both cases because P cannot be
      present*)
    assert (Hf1: formula_rep gamma_valid pd pf (mk_vt (s_params p) srts) vv f1
    (proj1' (typed_if_inv Hvalf)) =
    formula_rep gamma_valid pd (interp_with_Ps gamma_valid pd pf (map fst ps) Ps) (mk_vt (s_params p) srts) vv f1
    (proj1' (typed_if_inv Hvalf))). {
    ...
    rewrite <- Hf1.
    destruct (formula_rep gamma_valid pd pf (mk_vt (s_params p) srts) vv f1
  ...
  - (*Hmm, this is the hardest one - need rewrite lemma for match*)
    ...
    assert (Hteq: 
    (term_rep gamma_valid pd pf (mk_vt (s_params p) srts) vv t ty Hvalf) =
    (term_rep gamma_valid pd (interp_with_Ps gamma_valid pd pf (map fst ps) Ps) (mk_vt (s_params p) srts) vv t ty
        Hvalf)). {
    ...
    rewrite (match_val_single_change_gamma gamma_valid gamma_valid eq_refl pd
               (interp_with_Ps gamma_valid pd pf (map fst ps) Ps) pf) with (Hval2:=Forall_inv Hps) by auto.
    simpl.
    destruct (match_val_single gamma_valid pd pf (mk_vt (s_params p) srts) ty fh (Forall_inv Hps)
    (term_rep gamma_valid pd pf (mk_vt (s_params p) srts) vv t ty Hvalf)) eqn : Hm.
...
Lemma substi_mult_notin_eq (pf1 pf2: pi_funpred gamma_valid pd) (pf_eq: pf_same_constrs pf1 pf2)
...
Proof.
  ...
  (term_rep gamma_valid pd pf1 vt vv t (snd v) (Forall_inv Hall)) =
  (substi pd vt vv v
      (term_rep gamma_valid pd pf2 vt vv t (snd v) (Forall_inv Hall)))). {
...
Lemma preds_srts_rewrite (pf: pi_funpred gamma_valid pd) (p: predsym) srts1 srts2 (Heq: srts1 = srts2)
...
(*Show [pred_with_params] preserved through indpred_decomp*)
...
Theorem indpred_constrs_true
  ...
  formula_rep gamma_valid pd pf (mk_vt (s_params p) srts) (mk_vv _) f Hvalf.
Proof.
  assert (Hinfs': In fs (map snd indpred)). {
    rewrite in_map_iff. exists (p, fs); auto.
  }
  (*Part 1: work with alpha conversion to get wf*)
  rewrite (a_convert_all_f_rep gamma_valid _ _ aset_empty).
  ...
  assert (Hvalinda:=(a_convert_all_f_valid_ind_form p f aset_empty Hvalindf)).
  assert (Hwfa:=(a_convert_all_f_wf f aset_empty)).
  assert (Hposa:=(a_convert_all_f_pos (map fst indpred) f aset_empty Hposf)).
  (*Part 2: Work with [indpred_transform] *)
  rewrite indpred_decomp_equiv; auto.
  assert (Hvaldec:=(indpred_transform_valid _ (a_convert_all_f_typed _ aset_empty Hvalf))).
  ...
  rewrite fmla_rep_irrel with
    (Hval2:= (fforalls_typed (tup_1 (indpred_decomp (a_convert_all_f f aset_empty))) _ Hval1 Halltup1)).
  ...
  assert (Hval3: formula_typed gamma (Fpred p (map vty_var (s_params p)) 
    (snd (get_indprop_args (a_convert_all_f f aset_empty))))). {
  ...
  assert (Hformf: formula_rep gamma_valid pd
    (interp_with_Ps gamma_valid pd pf (map fst indpred) Ps)  (mk_vt (s_params p) srts)
    (mk_vv _) f Hvalf). {
      ...
      assert (Hex:=(in_dep_map 
                      (formula_rep gamma_valid pd (interp_with_Ps gamma_valid pd pf (map fst indpred) Ps)
                         (mk_vt (s_params p) srts) (mk_vv _)) _ Hallfs _ Hinf)).
      destruct Hex as [Hval4 Hinf'].
      erewrite fmla_rep_irrel. apply Hinf'.
  }
  (*Now we repeat the process again (alpha, [indpred_transform, etc])*)
  revert Hformf.
  rewrite a_convert_all_f_rep with(l:=aset_empty), indpred_decomp_equiv; auto.
  unfold indpred_transform.
  rewrite fmla_rep_irrel with
  ...
  assert ((substi_multi_let gamma_valid pd (interp_with_Ps gamma_valid pd pf (map fst indpred) Ps)
    (mk_vt (s_params p) srts) 
    (substi_mult pd (mk_vt (s_params p) srts) (mk_vv _) 
    (tup_1 (indpred_decomp (a_convert_all_f f aset_empty))) h)
    (tup_2 (indpred_decomp (a_convert_all_f f aset_empty))) Halltup2) =
  ...
  - f_equal. apply Denotational.get_arg_list_eq.
    rewrite Forall_forall. intros x Hinx ty Hty1 Hty2.
    rewrite H.
    rewrite term_rep_irrel with(Hty2:=Hty2).
    apply tm_change_pf; auto. apply pf_same_constrs_refl; auto.
    intros p1 srts' a' Hinp1. simpl.
    destruct (in_bool_spec predsym_eq_dec p1 (map fst indpred)); 
    [|rewrite find_apply_pred_notin; auto].
    (*Use fact that p1 not in x*)
    assert (Hindt: ind_positive (map fst indpred) (tup_4 (indpred_decomp (a_convert_all_f f aset_empty)))).
  ...
  - rewrite <- H0. apply Hformf.
    ...
    (substi_mult pd (mk_vt (s_params p) srts) (mk_vv (mk_vt (s_params p) srts))
       (tup_1 (indpred_decomp (a_convert_all_f f aset_empty))) h)
    (tup_2 (indpred_decomp (a_convert_all_f f aset_empty))) Halltup2) as vv'.
    clear Heqvv'.
    ...
    + (*Need to show that this is preserved*)
      ...
      apply indpred_decomp_and_pred_with_params with(f:=a_convert_all_f f aset_empty); auto.
...
Proof.
  ...
  - intros. (*Need to know x in s_params*)
    assert (Hinx: In x (s_params p)). {
      ...
      rewrite Forall_forall in Hsub.
      setoid_rewrite asubset_def in Hsub.
      specialize (Hsub f Hinf _ H). simpl_set; auto.
    }
    unfold mk_vt.
...
Theorem indpred_constrs_true_single
  ...
  (Hparams: Forall (fun f0 => pred_with_params_fmla [p] 
    (s_params p) f0) fs)
  (Hsub: Forall (fun f0 => 
    asubset (fmla_type_vars f0) (list_to_aset (s_params p))) fs)
  (srts: list sort)
  (srts_len: length srts = length (s_params p))
  (Hvt: vt_eq vt (s_params p) srts) :
...
Theorem indpred_least_pred_single (pf: pi_funpred gamma_valid pd) 
  ...
  (srts_len: length srts = length (s_params p))
  (Hvt: vt_eq vt (s_params p) srts) 
  (Hsub: Forall (fun f => asubset (fmla_type_vars f) (list_to_aset (s_params p))) fs) 
  (Hclosed: Forall closed_formula fs)
  (a: arg_list (domain (dom_aux pd)) (sym_sigma_args p srts)),
  (*If P holds of all of the constructors*)
  iter_and
  (map is_true
      (dep_map
        (formula_rep gamma_valid pd (interp_with_P gamma_valid pd pf p P)
          vt vv) fs Hform)) ->
(*Then indpred_rep p fs x -> P x*)  
...
(*Inprop rep does not depend on definition for
  ps in pf*)
Lemma indpred_rep_change_pf (pf1 pf2: pi_funpred gamma_valid pd) (pf_eq: pf_same_constrs pf1 pf2) 
(indpred : list (predsym * list formula))
(Hform: Forall (Forall (formula_typed gamma)) (map snd indpred)) 
(p: predsym)
(Hin: in_bool predsym_eq_dec p (map fst indpred))
...
Proof.
  ...
  (dep_map
     (formula_rep gamma_valid pd (interp_with_Ps gamma_valid pd pf1 preds Ps)
        (mk_vt (s_params p) srts) (mk_vv _)) a
    Hform) =
    (dep_map (formula_rep gamma_valid pd (interp_with_Ps gamma_valid pd pf2 preds Ps)
                (mk_vt (s_params p) srts) (mk_vv _)) a
        Hform)).
  { intros.
    apply dep_map_ext.
...
Definition indpred_rep_full (pf: pi_funpred gamma_valid pd)
  ...
  bool :=
  indpred_rep pf l
  (in_indpred_valid gamma_valid l_in) p p_in srts a.
...
Build_pi_funpred gamma_valid pd (funs gamma_valid pd pf)
  (pf_with_indprop_preds pf l l_in) (adt_props gamma_valid pd pf).
