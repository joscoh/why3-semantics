...
Fixpoint pat_names_uniq (p:pattern) : Prop :=
  ...
  | Pconstr f args ps =>
    ...
    i <> j ->
    forall x, ~ (aset_mem x (aset_map fst (pat_fv (List.nth i ps Pwild))) /\
      aset_mem x (aset_map fst (pat_fv (List.nth j ps Pwild))))
  | Por p1 p2 => pat_names_uniq p1 /\ pat_names_uniq p2 /\
    (forall x, aset_mem x (aset_map fst (pat_fv p1)) <-> aset_mem x (aset_map fst (pat_fv p2)))
  | Pbind p x => pat_names_uniq p /\ ~ aset_mem (fst x) (aset_map fst (pat_fv p))
  | _ => true
  end.
...
Fixpoint pat_names_uniqb (p: pattern) : bool :=
  ...
  | Pconstr f args ps => 
    ...
    (x != y) ==>  
    aset_is_empty (aset_intersect (aset_map fst (pat_fv (nth Pwild ps x))) 
      (aset_map fst (pat_fv (nth Pwild ps y))))]
  | Por p1 p2 => pat_names_uniqb p1 && pat_names_uniqb p2 &&
    (aset_eq_dec (aset_map fst (pat_fv p2)) (aset_map fst (pat_fv p1)))
  | Pbind p x => pat_names_uniqb p && 
    (negb (aset_mem_dec (fst x) (aset_map fst (pat_fv p))))
  | _ => true
  end.
...
Proof.
  ...
  - (*The hard case*)
    ...
    (x != y) ==>
    aset_is_empty (aset_intersect 
      (aset_map fst(pat_fv (nth Pwild ps x))) 
      (aset_map fst (pat_fv (nth Pwild ps y))))] 
    ...
    + apply ReflectF => [[_ C]]. apply Hforallx. move=> x.
      ...
      destruct (aset_is_empty _) eqn : Hemp; auto.
      rewrite aset_is_empty_false in Hemp.
      destruct Hemp as [z Hmemz].
      simpl_set. exfalso.
    ...
    + apply ReflectT. split=>//. move => i j /ltP Hi /ltP Hj /eqP Hij x.
      ...
      move => /(_ Hneq) Hemp.
      eapply aset_is_empty_mem in Hemp. apply Hemp.
      simpl_set. eauto.
  - by reflT.
  - move=> p1 p2 Hr1 Hr2. rewrite -andbA. apply andPP=>//.
    apply andPP=>//.
    case: (aset_eq_dec (aset_map fst (pat_fv p2)) (aset_map fst (pat_fv p1))) => Heq/=.
    + apply ReflectT. by rewrite Heq.
    + apply ReflectF => C. 
      apply Heq. apply aset_ext. intros x. by rewrite C.
  - move=> p1 v Hr.
    apply andPP=>//.
    apply negPP.
    case: (aset_mem_dec v.1 (aset_map fst (pat_fv p1))) => Hmem /=.
    + by apply ReflectT.
    + by apply ReflectF.
Qed.
...
Lemma pat_names_uniq_nodup gamma (p: pattern) ty:
  ...
  NoDup (map fst (aset_to_list (pat_fv p))) ->
  pat_names_uniq p.
Proof.
  revert ty. induction p; simpl; intros; auto.
  - split.
    { rewrite -> Forall_map, Forall_forall. intros.
      apply nodup_map_aset_big_union_inv with (x:=x) in H1; auto.
      (* 
    ...
    eapply nodup_map_aset_big_union_inv' in H1. apply H1.
    all: auto.
  - inversion H; subst.
    apply nodup_map_aset_union_inv in H0; try apply NoDup_pat_fv.
    destruct H0.
    split_all; eauto.
    intros. simpl_set. setoid_rewrite H7. reflexivity.
  - inversion H; subst. 
    assert (Hn:=H0).
    apply nodup_map_aset_union_inv in H0; destruct_all.
    split; eauto.
    eapply nodup_map_aset_union_inv' in Hn; auto.
    2: { intros y [Hiny1 Hiny2]; simpl_set; subst; auto. }
    intro C.
    apply Hn. split. apply C. simpl_set. exists v. simpl_set; auto.
Qed.
...
(forall x, aset_mem x (pat_fv (ty_subst_p p)) <-> aset_mem x (aset_map ty_subst_var (pat_fv p))).
Proof.
  intros. induction p; simpl; auto; try reflexivity.
  - simpl_set. split.
    + intros Hx; subst. exists v. simpl_set; auto.
    + intros [y [Hy1 Hy2]]; simpl_set; subst; auto.
  - simpl_set.
    split.
    + intros [p [Hinp Hinx]].
      ...
      rewrite Forall_forall in H.
      specialize (H _ Hinp').
      destruct H. specialize (H Hinx); clear H0. simpl_set.
...
Proof.
  ...
  - subst sigma. rewrite ty_subst_twice; auto; [| apply s_params_Nodup].
    ...
    + rewrite !length_map. intros.
      ...
      apply (Hdisj i j H0 H1 H2 (fst x0)). simpl_set.
      split; [exists x1 | exists x0]; auto.
  - constructor. apply valid_type_ty_subst'; auto.
  - simpl in Hp2. destruct Hp2. destruct H0. constructor.
    + apply IHp1; auto.
    + apply IHp2; auto.
    + apply aset_ext. intros. rewrite !ty_subst_p_fv. rewrite H5; reflexivity.
  - simpl in Hp2. destruct Hp2.
    constructor.
    + rewrite ty_subst_p_fv. simpl_set. intros C; destruct_all.
      apply H0. simpl_set. exists x; split; auto.
...
Definition tm_wf_strong t :=
  NoDup (map fst (aset_to_list (tm_fv t))) /\
  NoDup (map fst (tm_bnd t)) /\
  disj (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t)).

Definition fmla_wf_strong f :=
  NoDup (map fst (aset_to_list (fmla_fv f))) /\
  NoDup (map fst (fmla_bnd f)) /\
  disj (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f)).
...
Proof.
  ...
  - rewrite Forall_map.
    ...
    + (*Lots of unfolding to show that the pat fvs are actually NoDup*) 
      ...
      specialize (H10 (map fst (aset_to_list (pat_fv x.1) ++ tm_bnd x.2)%list)).
      prove_hyp H10.
      {
  ...
  - (*Match same*)
    ...
    + (*Lots of unfolding to show that the pat fvs are actually NoDup*) 
      ...
      apply H6; auto.
      rewrite !map_map in H10.
      specialize (H10 (map fst (aset_to_list (pat_fv x.1) ++ fmla_bnd x.2)%list)).
      prove_hyp H10.
      {
        rewrite in_map_iff. eexists; split; [reflexivity | auto].
      }
...
Lemma match_val_single_change_ty pd pf vt (ty1 ty2: vty) 
  (p: pattern)
...
Lemma match_val_single_ty_subst pd pf vt (ty: vty) 
  (p: pattern)
  ...
  opt_related 
    (fun m1 m2 =>
      forall x y, 
      amap_lookup m2 x = Some y ->
      exists z Heq,
      amap_lookup m1 (ty_subst_var x) = Some z /\
      projT2 z = dom_cast (dom_aux pd) Heq (projT2 y))
    (match_val_single gamma_valid pd pf vt (ty_subst' params tys ty) 
      (ty_subst_p p) Hp2 (dom_cast (dom_aux pd) Heq d))
    (match_val_single gamma_valid pd pf
      (vt_with_args vt params (map (v_subst vt) tys)) ty p Hp1 d).
Proof.
  revert ty vt Hp1 Hp2 Heq d.
  induction p as [v1 | f1 tys1 ps1 IH | | p1 q1 IH1 IH2 | p1 v1 IH]; intros; auto; try reflexivity.
  - (*Pvar (base case)*) simpl. intros x y Hlookup.
    apply lookup_singleton_iff in Hlookup.
    destruct Hlookup as [Hx Hy]; subst. simpl. unfold amap_singleton. 
    setoid_rewrite amap_set_lookup_same.
    eexists. eexists. split; [reflexivity|]. simpl. reflexivity.
  - (*Pconstr - hard case*)
    rewrite !match_val_single_rewrite.
    revert Hp2. Opaque amap_lookup. cbn. intros Hp2. 
    ...
    2: {
      ...
      assert (Hisadt2: is_vty_adt gamma (ty_subst' params tys ty) = None).
      { apply (constr_pat_is_vty_adt_none Hp1 Hp2), Hisadt1. }
      rewrite Hisadt2. simpl; auto.
    }
    destruct p as [[m1 a1] vs1].
    assert (Hvs1: vs1 = tys1). {
    ...
    assert (Hconstrs:
             ADTSpec.constr_rep gamma_valid (dom_aux pd) (funs gamma_valid pd pf) m_in a_in x_in2 e0 args2 =
               dom_cast (dom_aux pd) (f_equal (fun x => s_cons (adt_name a1) x) (Logic.eq_sym Heq2))
            ( ADTSpec.constr_rep gamma_valid (dom_aux pd) (funs gamma_valid pd pf) m_in a_in x_in1 e args1)).
    { rewrite  <- Hcast1, <- Hcast2. rewrite !dom_cast_compose. apply dom_cast_eq. }
    clear Hcast1 Hcast2.
    (*Need to use [constrs_inj_strong] separately for dep types*)
    assert (Hinj: exists (Heq : f1' = f2),
               args1 = cast_arg_list
                         (f_equal2 (fun (x: funsym) (srts: list sort) => sym_sigma_args x srts)
                            (Logic.eq_sym Heq) Heq2) args2).
    {
      (*Get rid of cast in Hconstrs - cant' do in general bc of dep types*)
      generalize dependent (map (v_subst vt) (ty_subst_list' params tys tys1)).
      intros; subst. simpl in Hconstrs |- *. assert (e = e0) by (apply UIP_dec, Nat.eq_dec); subst.
      apply constrs_inj_strong in Hconstrs.
      destruct Hconstrs as [Hf Hargs]. subst. exists Logic.eq_refl. simpl.
      reflexivity.
    }
    destruct Hinj as [Hf Hargs]. clear Hconstrs.
    subst f1'.
    assert (x_in2 = x_in1) by (apply bool_irrelevance); subst.
    (*Now that we know all of this information, we can simplify for induction*)
    destruct (funsym_eq_dec f2 f1); [| solve[simpl; auto]]. subst.
    (*Deal with Hvslen1*)
    repeat match goal with
    ...
    intros ? ?.
    assert (Hcast: cast_arg_list e4 args2 = cast_arg_list (eq_trans (Logic.eq_sym e3) e4) (cast_arg_list e3 args2)).
    ...
    clear e3 e4 e1 e2. intros ? a3. clear args2.
    (*Now generalize for induction*)
    match goal with
    | |- opt_related ?P (iter_arg_list ?val ?pd ?pf ?l1 ?a1 ?ps1 ?H1)
        (iter_arg_list ?val ?pd ?pf ?l2 ?a2 ?ps2 ?H2) =>
      generalize dependent H1;
      generalize dependent H2
    end.
    unfold sym_sigma_args in *.
    assert (Hlenvs: length tys1 = length (s_params f1)). {
      inversion Hp1; subst; auto.
    }
    (*Need disjointness of variables*)
    pose proof (Hdisj:=pat_constr_disj_map Hp2).
    assert (Hlen: length (s_args f1) = length ps1). {
      inversion Hp1; subst. auto.
    }
    ...
    induction l as [| p1 ps IHps]; intros.
    + destruct ps1; try discriminate.
    + destruct ps1; try discriminate; simpl; auto.
      inversion IH as [| ? ? IH1 IH2]; subst.
      (*Simplify IH before destructing*)
      rewrite -> hlist_hd_cast with (Heq2:=cons_inj_hd e1).
      (*Need a cast*)
      assert (Heqty: ty_subst (s_params f1) (ty_subst_list' params tys tys1) p1 =
      ty_subst' params tys (ty_subst (s_params f1) tys1 p1)).
      { rewrite ty_subst_twice; auto. apply s_params_Nodup. }
      assert (Hp2': pattern_has_type gamma (ty_subst_p p) (ty_subst' params tys (ty_subst (s_params f1) tys1 p1))).
      { rewrite <- Heqty. inversion f0; subst; auto. }
      erewrite -> match_val_single_change_ty with
        (ty2:=(ty_subst' params tys (ty_subst (s_params f1) tys1 p1)))
        (Heq:=Heqty)  (Hp2:=Hp2').
      rewrite -> rewrite_dom_cast, !dom_cast_compose.
      specialize (IH1 (ty_subst (s_params f1) tys1 p1) vt (Forall_inv f) Hp2' 
        (eq_trans (cons_inj_hd e1) (f_equal (v_subst vt) Heqty)) (hlist_hd a3)).
      (*Now we have IH info in IH1 for first two matches*)
      destruct (match_val_single _ _ _ _ _ p _ _) as [a1|] eqn : Hmatch1;
      destruct (match_val_single _ _ _ _ _ (ty_subst_p p) _ _) as [a2|] eqn : Hmatch2;
      simpl in IH1 |- *; auto; try contradiction.
      (*Now simplify IH for [iter_arg_list]*)
      rewrite -> hlist_tl_cast. clear IH. simpl in Hdisj.
      rewrite disj_map_cons_iff' in Hdisj. destruct Hdisj as [Hdisj1 Hdisj2].
      specialize (IHps (cons_inj_tl e1) (hlist_tl a3) _ IH2 Hdisj1 ltac:(auto) (Forall_inv_tail f) (Forall_inv_tail f0)). clear IH2.
      destruct (iter_arg_list _ _ _ _ _ ps1 _) as [b1|] eqn : Hmatch3;
      destruct (iter_arg_list _ _ _ _ _ (map ty_subst_p ps1) _) as [b2|] eqn : Hmatch4;
      simpl in IHps |- *; auto; try contradiction.
      (*Remaining - just combine the maps*)
      intros x y Hlookup.
      rewrite amap_union_lookup in Hlookup.
      destruct (amap_lookup a1 x) as [z|] eqn : Hlook1.
      * inversion Hlookup; subst. apply IH1 in Hlook1.
        destruct Hlook1 as [z [Heq [Hlookz Hz]]].
        exists z. exists Heq. rewrite amap_union_lookup. rewrite Hlookz. auto.
      * apply IHps in Hlookup. (*TODO: think I need iff*)
        destruct Hlookup as [z [Heq [Hz1 Hz2]]].
        exists z. exists Heq. rewrite amap_union_lookup.
        (*really just need disjoint: a2 and b2 must have disjoint variables*)
        (*Here, use free var result*)
        destruct (amap_lookup a2 (ty_subst_var x)) eqn : Hlooka2.
        { (*Here, use disjointness*)
          assert (Hinfv1: aset_mem (ty_subst_var x) (pat_fv (ty_subst_p p))).
          { erewrite <- match_val_single_fv; eauto. rewrite <- amap_mem_keys.
            rewrite amap_mem_spec. rewrite Hlooka2; auto.
          }
          assert (Hinfv2: aset_mem (ty_subst_var x) (aset_big_union pat_fv (map ty_subst_p ps1))).
          {
            simpl in Hmatch4. simpl in *.
            erewrite -> iter_arg_list_matches_row in Hmatch4. apply matches_row_vars in Hmatch4.
            rewrite <- Hmatch4. rewrite <- amap_mem_keys, amap_mem_spec. rewrite Hz1. auto.
            Unshelve.
            (*Prove row typed*)
        ...
        auto.
  - (*Pwild case*)
    simpl. intros x y. rewrite amap_empty_get; discriminate.
  - (*Por*)
    simpl.
    specialize (IH1 ty vt (proj1' (pat_or_inv Hp1)) (proj1' (pat_or_inv Hp2)) Heq d).
    specialize (IH2 ty vt (proj2' (pat_or_inv Hp1)) (proj2' (pat_or_inv Hp2)) Heq d).
    destruct (match_val_single _ _ _ _ _ p1 _ _) as [a1|] eqn : Hmatch1;
    destruct (match_val_single _ _ _ _ _ (ty_subst_p p1) _ _) as [a2|] eqn : Hmatch2;
    simpl in IH1 |- *; auto; contradiction.
  - (*Pbind*)
    simpl.
    specialize (IH ty vt (proj1' (pat_bind_inv Hp1)) (proj1' (pat_bind_inv Hp2)) Heq d).
    destruct (match_val_single _ _ _ _ _ p1 _ _) as [a1|] eqn : Hmatch1;
    destruct (match_val_single _ _ _ _ _ (ty_subst_p p1) _ _) as [a2|] eqn : Hmatch2;
    simpl in IH |- *; auto; try contradiction.
    (*Will need disjointness later probably*)
    intros x y Hlookup. rewrite amap_set_lookup_iff in Hlookup.
    destruct Hlookup as [[Hx Hy] | [Hnotx Hy]].
    + subst. simpl. setoid_rewrite amap_set_lookup_same.
      eexists. eexists. split; reflexivity.
    + apply IH in Hy. destruct Hy as [z [Heq1 [Hlookz Hy]]]. 
      vsym_eq (ty_subst_var x) (ty_subst_var v1).
      { (*Another contradiction*)
        exfalso. inversion Hp2; subst. rewrite <- e in H3.
        apply H3. erewrite <- match_val_single_fv; eauto.
        rewrite <- amap_mem_keys, amap_mem_spec. rewrite Hlookz. auto.
      }
      setoid_rewrite amap_set_lookup_diff; eauto.
Qed.
...
Lemma ty_subst_tf_rep {pd: pi_dom} {pf: pi_funpred gamma_valid pd}
  ...
  (forall (vt: val_typevar) (vv1: val_vars pd vt)
    ...
    (Hvv: forall x Heq, aset_mem x (tm_fv t) -> vv2 x = dom_cast (dom_aux pd) Heq 
      (vv1 (ty_subst_var x)))
    Heq,
    (*[term_rep] version is ugly because of cast, but we mostly
      need formula version (or maybe not, see)*)
    term_rep gamma_valid pd pf vt vv1 (ty_subst_tm t) ty2 Hty2 =
    dom_cast (dom_aux pd) Heq (term_rep gamma_valid pd pf
  ...
  (vv2: val_vars pd (vt_with_args vt params (map (v_subst vt) tys)))
    ...
    (Hvv: forall x Heq, aset_mem x (fmla_fv f) -> vv2 x = dom_cast (dom_aux pd) Heq 
      (vv1 (ty_subst_var x))),
    formula_rep gamma_valid pd pf vt vv1 (ty_subst_fmla f) Hty2 =
    formula_rep gamma_valid pd pf
      (vt_with_args vt params (map (v_subst vt) tys)) vv2 f Hty1).
Proof.
  unfold tm_wf_strong_rec, fmla_wf_strong_rec.
  ...
  - (*Variable case - more casting*)
    ...
    apply dom_cast_eq. simpl_set; auto.
  - (*Function case - hard because of casting already and
    ...
    (term_rep gamma_valid pd pf vt vv1) Hty2) =
    (fun_arg_list pd (vt_with_args vt params (map (v_subst vt) tys)) f1 l l1
        (term_rep gamma_valid pd pf (vt_with_args vt params (map (v_subst vt) tys))
           vv2) Hty1)).
    {
      unfold fun_arg_list.
  ...
  - (*Tif*)
    ...
    destruct (formula_rep gamma_valid pd pf (vt_with_args vt params (map (v_subst vt) tys)) vv2 f
  ...
  - (*Tmatch*)
      ...
      x = y).
    {
      ...
      vsym_eq x y.
      destruct (aset_mem_dec y (pat_fv p)).
      {
      ...
      - rewrite in_app_iff. right.
        ...
        apply Hinpt. rewrite in_app_iff; simpl_set; auto.
    }
    ...
    simpl.
    (*Now use [match_val_single] lemma*)
    pose proof (match_val_single_ty_subst pd pf vt v p (Forall_inv Hpat1)
       (Forall_inv Hpat2) Heq1 
        (term_rep gamma_valid pd pf (vt_with_args vt params (map (v_subst vt) tys)) vv2 tm v Hty1)) as Hmatch.
    destruct (match_val_single _ _ _ _ _ p _ _) as [a1|] eqn : Hmatch1;
    destruct (match_val_single _ _ _ _ _ (ty_subst_p p) _ _) as [a2|] eqn : Hmatch2;
    simpl in Hmatch |- *; auto; try contradiction.
    + (*Both Some*)
      apply H3.
      ...
      2: {
        ...
        - rewrite amap_mem_keys_false. erewrite match_val_single_fv; eauto.
          rewrite ty_subst_p_fv. simpl_set.
          intros [y [Hxy Hiny]].
      ...
      (*So now we know that x is in (pat_fv p), and
        therefore we know that it is in a1)*)
      assert (Hina1: amap_mem x a1). {
        rewrite amap_mem_keys. erewrite match_val_single_fv; eauto.
      }
      rewrite amap_mem_spec in Hina1.
      destruct (amap_lookup a1 x) as [y|] eqn : Hlook; [|discriminate]. assert (Hlook':=Hlook).
      apply Hmatch in Hlook.
      destruct Hlook as [z [Heqyz [Hz Hy]]].
      rewrite -> extend_val_lookup with (t:=y) by auto.
      rewrite -> extend_val_lookup with (t:=z) by auto.
      (*Now just a lot of casting*)
      assert (projT1 y = (v_subst (vt_with_args vt params (map (v_subst vt) tys)) x.2)).
      {
        eapply match_val_single_typs. apply Hmatch1. auto.
      }
      destruct (sort_eq_dec (v_subst (vt_with_args vt params (map (v_subst vt) tys)) x.2) (projT1 y));
      try (exfalso; apply n; auto).
      assert (projT1 z = (v_subst vt (ty_subst_var x).2)). {
        rewrite <- Heqyz. rewrite v_subst_vt_with_args'; auto.
      }
      destruct (sort_eq_dec (v_subst vt (ty_subst_var x).2) (projT1 z));
      [| exfalso; apply n; auto].
      rewrite Hy. rewrite !dom_cast_compose. apply dom_cast_eq.
    + (*Both None - IH*)
      rewrite <- H with(vv1:=vv1)(Hty2:=Hty2). auto. simpl. 
      apply IHps; auto. all: auto.
  ...
  - (*Fquant*)
    ...
    assert (forall d1 d2,
      ...
      formula_rep gamma_valid pd pf vt (substi pd vt vv1 (ty_subst_var v) d2) 
      (ty_subst_fmla f) (typed_quant_inv Hty2) =
    formula_rep gamma_valid pd pf (vt_with_args vt params (map (v_subst vt) tys))
  ...
  - (*Fmatch*)
      ...
      x = y).
    {
      ...
      rewrite map_app. split.
      - rewrite !in_map_iff. exists y.
        split; auto. simpl. simpl_set. right. exists (p, t); split; auto.
        simpl_set; auto.
      - rewrite in_app_iff. right.
        rewrite in_map_iff. exists x. split; auto.
        rewrite in_concat. eexists.
...
Lemma subterm_fv_in x tm t f:
  (In tm (subterms_t t) -> 
    aset_mem x (tm_fv tm) -> aset_mem x (tm_fv t) \/
    In x (tm_bnd t)) /\
  (In tm (subterms_f f) -> 
  aset_mem x (tm_fv tm) -> aset_mem x (fmla_fv f) \/
  In x (fmla_bnd f)).
Proof.
  revert t f. apply term_formula_ind; simpl; intros; 
...
Lemma subfmla_fv_in x f1 t f:
  (In f1 (subfmla_t t) -> 
    aset_mem x (fmla_fv f1) -> aset_mem x (tm_fv t) \/
    In x (tm_bnd t)) /\
  (In f1 (subfmla_f f) -> 
  aset_mem x (fmla_fv f1) -> aset_mem x (fmla_fv f) \/
  In x (fmla_bnd f)).
Proof.
  revert t f. apply term_formula_ind; simpl; intros; 
  destruct_all; try contradiction; simpl in *;
...
Proof.
  ...
  - destruct H1; subst; auto.
    ...
    specialize (Hn2 (map fst (aset_to_list (pat_fv (fst x0)) ++ tm_bnd (snd x0)))).
    prove_hyp Hn2.
  ...
  - rewrite map_app in Hbnd.
    ...
    rewrite -> Forall_map, Forall_forall in H0.
    eapply H0. apply H3. all: auto.
    specialize (Hn2 (map fst (aset_to_list (pat_fv (fst x0)) ++ fmla_bnd (snd x0)))).
    prove_hyp Hn2.
    { rewrite in_map_iff. exists x0; auto. }
    rewrite -> map_app, NoDup_app_iff in Hn2.
    apply Hn2.
...
Lemma subterm_disj tm t f:
  (forall (Hn: NoDup (map fst (tm_bnd t)))
    (Hd: disj (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t)))
    (Hsub: In tm (subterms_t t)),
    disj (map fst (aset_to_list (tm_fv tm))) (map fst (tm_bnd tm))) /\
  (forall (Hn: NoDup (map fst (fmla_bnd f)))
    (Hd: disj (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f)))
...
Proof.
  ...
  - destruct Hsub as [Hsub | Hsub]; subst; auto.
    ...
    + apply (disj_sublist_lr Hd); apply sublist_map; 
      [rewrite sublist_aset_to_list; apply asubset_big_union | apply sublist_concat_map]; auto.
  - (*Tlet*) destruct Hsub as [Hsub | Hsub]; subst; auto.
    ...
    + apply (disj_sublist_lr Hd).
      * apply sublist_map. rewrite sublist_aset_to_list. apply union_asubset_l.
  ...
  - (*Tif*)
    ...
    apply (disj_sublist_lr Hd); apply sublist_map.
    + rewrite sublist_aset_to_list. apply union_asubset_l.
    + apply sublist_app_l.
    + rewrite sublist_aset_to_list. eapply asubset_trans. apply union_asubset_l.
      apply union_asubset_r.
    + eapply sublist_trans. apply sublist_app_l.
      apply sublist_app_r.
    + rewrite sublist_aset_to_list. eapply asubset_trans. apply union_asubset_r.
      apply union_asubset_r.
  ...
  - (*Tmatch case*)
    ...
    + clear H Hn1. rewrite in_concat in Hsub.
      ...
      destruct Hn2 as [Hn2 _].
      specialize (Hn2 (map fst (aset_to_list (pat_fv pt.1) ++ tm_bnd pt.2))).
      prove_hyp Hn2.
      ...
      destruct (in_dec string_dec x (map fst (aset_to_list (pat_fv pt.1)))).
      ...
      * (*Otherwise, use disj assumption*)
        ...
        -- rewrite in_map_iff. exists v1. split; auto.
          ...
          apply n. rewrite in_map_iff. exists v1. simpl_set; auto.
        -- rewrite in_map_iff. exists v2. split; auto.
          rewrite in_app_iff. right. rewrite in_concat.
          exists (aset_to_list (pat_fv pt.1) ++ tm_bnd pt.2).
          split; auto.
          ++ rewrite in_map_iff. exists pt; auto.
  ...
  - (*Feq*)
    ...
    apply (disj_sublist_lr Hd); apply sublist_map; try rewrite sublist_aset_to_list;
    [apply union_asubset_l | apply sublist_app_l |
      apply union_asubset_r | apply sublist_app_r].
  - (*Fbinop*)
    rewrite -> map_app, NoDup_app_iff in Hn.
    destruct Hn as [Hn1 [Hn2 _]].
    rewrite in_app_iff in Hsub.
  ...
  - (*Fif*)
    ...
    + apply sublist_app_l.
    + eapply asubset_trans. apply union_asubset_l.
      apply union_asubset_r.
    + eapply sublist_trans. apply sublist_app_l.
      apply sublist_app_r.
  ...
  - (*Tmatch case*)
    ...
    + clear H Hn1. rewrite in_concat in Hsub.
      ...
      rewrite -> concat_map, !map_map, NoDup_concat_iff in Hn2.
      destruct Hn2 as [Hn2 _].
      specialize (Hn2 (map fst (aset_to_list (pat_fv pt.1) ++ fmla_bnd pt.2))).
      prove_hyp Hn2.
      {  rewrite in_map_iff. exists pt; auto. }
      rewrite -> map_app, NoDup_app_iff in Hn2.
      destruct Hn2 as [Hn1 [Hn2 [Hnotin _]]]. 
      ...
      * (*Otherwise, use disj assumption*)
        ...
        -- rewrite in_map_iff. exists v2. split; auto.
          rewrite in_app_iff. right. rewrite in_concat.
          exists (aset_to_list (pat_fv pt.1) ++ fmla_bnd pt.2).
          split; auto.
          ++ rewrite in_map_iff. exists pt; auto.
          ++ rewrite in_app_iff; auto.
Qed.
...
Lemma subfmla_disj f1 t f:
  (forall (Hn: NoDup (map fst (tm_bnd t)))
    ...
    disj (map fst (aset_to_list (fmla_fv f1))) (map fst (fmla_bnd f1))) /\
  (forall (Hn: NoDup (map fst (fmla_bnd f)))
    (Hd: disj (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f)))
...
Lemma fv_nodup_subterm tm t f:
  (forall (Hbnd: NoDup (map fst (tm_bnd t))) 
    (Hfv: NoDup (map fst (aset_to_list (tm_fv t))))
    (Hdisj: disj (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t)))
    (Hsub: In tm (subterms_t t)),
    NoDup (map fst (aset_to_list (tm_fv tm)))) /\
  (forall (Hbnd: NoDup (map fst (fmla_bnd f))) 
    (Hfv: NoDup (map fst (aset_to_list (fmla_fv f))))
    (Hdisj: disj (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f)))
    (Hsub: In tm (subterms_f f)),
    NoDup (map fst (aset_to_list (tm_fv tm)))).
Proof.
  split; intros;
  apply NoDup_map_inj; try apply aset_to_list_nodup;
  intros x y Hinx Hiny Hxy; simpl_set;
  [apply subterm_t_fv_in with(t:=t) in Hinx, Hiny |
    apply subterm_f_fv_in with(f:=f) in Hinx, Hiny]; auto;
  destruct Hinx; destruct Hiny;
  try solve[apply (NoDup_map_in Hbnd); simpl_set; auto];
  try solve[apply (NoDup_map_in Hfv); simpl_set; auto];
  exfalso; apply (Hdisj (fst x));
  split; rewrite in_map_iff;
  try solve[exists x; simpl_set; auto];
  solve[exists y; simpl_set; auto].
Qed.
...
Lemma fv_nodup_subfmla f1 t f:
  (forall (Hbnd: NoDup (map fst (tm_bnd t))) 
    ...
    (Hdisj: disj (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t)))
    (Hsub: In f1 (subfmla_t t)),
    NoDup (map fst (aset_to_list (fmla_fv f1)))) /\
  (forall (Hbnd: NoDup (map fst (fmla_bnd f))) 
    (Hfv: NoDup (map fst (aset_to_list (fmla_fv f))))
    (Hdisj: disj (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f)))
    (Hsub: In f1 (subfmla_f f)),
...
Proof.
  ...
  - intros x [Hinfv Hinbnd].
    ...
    destruct_all. simpl_set. exists x0. split; auto.
...
Definition make_tm_wf (t: term) : term :=
  if seq.uniq (map fst (tm_bnd t)) && disjb (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t))
  then t else a_convert_all_t t aset_empty.
...
Proof.
  ...
  case: (disjb (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t))) /disjP => Hdisj/=;
  last by apply a_convert_all_t_strong_wf.
...
Proof.
  unfold make_tm_wf.
  destruct (seq.uniq (map fst (tm_bnd t)) && disjb (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t))); auto.
  intros. apply a_convert_all_t_ty; auto.
Qed.

Lemma make_tm_wf_rep (pd: pi_dom)
  ...
  term_rep gamma_valid pd pf vt vv (make_tm_wf t) ty Hty2 =
  term_rep gamma_valid pd pf vt vv t ty Hty1.
Proof.
  revert Hty2.
  unfold make_tm_wf.
  destruct (seq.uniq (map fst (tm_bnd t)) && disjb (map fst (aset_to_list (tm_fv t))) (map fst (tm_bnd t)));
  intros.
...
Definition make_fmla_wf (f: formula) : formula :=
  if seq.uniq (map fst (fmla_bnd f)) && 
    disjb (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f))
  then f else a_convert_all_f f aset_empty.
...
Proof.
  ...
  case: (disjb (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f))) /disjP => Hdisj/=;
  last by apply a_convert_all_f_strong_wf.
...
Proof.
  ...
  destruct (seq.uniq (map fst (fmla_bnd f)) && 
    disjb (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f))); auto.
  intros. apply a_convert_all_f_typed; auto.
Qed.

Lemma make_fmla_wf_rep (pd: pi_dom)
  (pf: pi_funpred gamma_valid pd)
  (vt: val_typevar) (vv: val_vars pd vt) f
  (Hty1: formula_typed gamma f)
  (Hty2: formula_typed gamma (make_fmla_wf f)):
  formula_rep gamma_valid pd pf vt vv (make_fmla_wf f) Hty2 =
...
(*And now the full spec: given a formula with no duplicate
  ...
  substitution, this is the same as changing the type
  variable valuation accordingly and evaluating the original term*)
Theorem ty_subst_wf_fmla_rep (pd: pi_dom) 
  (pf: pi_funpred gamma_valid pd)
  (vt: val_typevar) (vv: val_vars pd vt) f 
  (Hty1: formula_typed gamma f)
  (Hty2: formula_typed gamma (ty_subst_wf_fmla f)):
  NoDup (map fst (aset_to_list (fmla_fv f))) ->
  formula_rep gamma_valid pd pf vt vv (ty_subst_wf_fmla f) Hty2 =
  formula_rep gamma_valid pd pf
    (vt_with_args vt params (map (v_subst vt) tys))
    (upd_vv_args params tys params_len params_nodup pd vt vv) f Hty1.
Proof.
  intros Hn.
...
Lemma type_vars_subst_sort ty:
  ...
  aset_is_empty (type_vars (ty_subst' params tys ty)).
Proof.
  intros. induction ty; simpl; auto.
  - simpl in H. specialize (H v (ltac:(simpl_set;auto))).
  ...
  - rewrite aset_big_union_empty. unfold is_true; rewrite forallb_forall.
    setoid_rewrite in_map_iff. intros x [y [Hx Hiny]].
    subst. rewrite Forall_forall in H1; apply H1; auto.
    simpl in H. intros x Hinx. apply H. simpl_set; eauto.
Qed.

Lemma ty_subst_srts_vars_p p
  (Hallin: forall x, aset_mem x (pat_type_vars p) -> In x params)
  (Hsrts: forall t, In t tys -> is_sort t):
  aset_is_empty (pat_type_vars (ty_subst_p p)).
Proof.
  induction p; simpl; auto.
  - apply type_vars_subst_sort; auto.
  - rewrite aset_union_empty andb_true !aset_big_union_empty. 
    unfold is_true; rewrite !forallb_forall. split.
    + intros ty.
      unfold ty_subst_list'.
      rewrite in_map_iff.
    ...
    + intros p.
      ...
      right.
      exists p1; auto.
  - simpl in *. setoid_rewrite aset_mem_union in Hallin. 
    rewrite -> aset_union_empty, IHp1, IHp2; auto.
  - simpl in *. setoid_rewrite aset_mem_union in Hallin.
    rewrite -> aset_union_empty, IHp; auto.
    simpl. apply type_vars_subst_sort; auto; intros; apply Hallin; simpl;
      simpl_set; auto.
Qed.
...
Lemma ty_subst_srts_vars
  ...
  (forall (Hallin: forall x, aset_mem x (tm_type_vars t) -> In x params),
    aset_is_empty (tm_type_vars (ty_subst_tm t))) /\
  (forall (Hallin: forall x, aset_mem x (fmla_type_vars f) -> In x params),
    aset_is_empty (fmla_type_vars (ty_subst_fmla f))).
Proof.
  revert t f; apply term_formula_ind; intros; auto.
  - apply type_vars_subst_sort; auto.
  - cbn. rewrite -> aset_union_empty, !aset_big_union_empty, !andb_true.
    unfold is_true; rewrite !forallb_forall. split.
    + unfold ty_subst_list'.
  ...
  - simpl. simpl in Hallin. repeat (setoid_rewrite aset_mem_union in Hallin).
    rewrite -> !aset_union_empty, H, H0; auto.
    simpl. apply type_vars_subst_sort; auto.
  - simpl. simpl in Hallin. repeat (setoid_rewrite aset_mem_union in Hallin).
    rewrite -> !aset_union_empty, H, H0, H1; auto.
  - simpl in Hallin. repeat (setoid_rewrite aset_mem_union in Hallin).
    simpl. rewrite -> !aset_union_empty, H; auto. simpl.
    rewrite -> !aset_big_union_empty,  type_vars_subst_sort; auto.
    rewrite -> andb_true_r, andb_true. unfold is_true; split; rewrite forallb_forall.
    + intros p.
      rewrite in_map_iff.
  ...
  - cbn in *.
    repeat (setoid_rewrite aset_mem_union in Hallin).
    rewrite -> aset_union_empty,H, type_vars_subst_sort; auto.
  - cbn in *. repeat (setoid_rewrite aset_mem_union in Hallin).
    rewrite -> aset_union_empty, !aset_big_union_empty, !andb_true.
    unfold is_true; rewrite !forallb_forall. split.
    + unfold ty_subst_list'.
      intros ty. rewrite in_map_iff.
      intros [ty' [Hty Hinty']]; subst.
  ...
  - cbn in *.
    ...
    rewrite -> aset_union_empty,H, type_vars_subst_sort; auto.
  - simpl. simpl in Hallin. repeat (setoid_rewrite aset_mem_union in Hallin).
    rewrite -> !aset_union_empty, H, H0, type_vars_subst_sort; auto.
  - simpl. simpl in Hallin. repeat (setoid_rewrite aset_mem_union in Hallin).
    rewrite -> !aset_union_empty, H, H0; auto.
  - simpl. simpl in Hallin. repeat (setoid_rewrite aset_mem_union in Hallin).
    rewrite -> !aset_union_empty, H, H0, type_vars_subst_sort; auto.
...
Proof.
  unfold make_fmla_wf.
  destruct ( seq.uniq (map fst (fmla_bnd f)) && disjb (map fst (aset_to_list (fmla_fv f))) (map fst (fmla_bnd f)));
  auto.
  symmetry.
