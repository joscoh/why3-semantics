...
(*START: prove this, define termination function that is a bit better,
  prove*)
...
Lemma funpred_in {A B T: Type} (sym: A -> B) (in_tm: B -> C -> bool):
  forall (fs: list A) (t: T),
  reflect (forall (f: A), In f fs -> negb (in_tm (sym f) t))
    (all (fun f => negb (in_tm (sym f) t)) fs)
