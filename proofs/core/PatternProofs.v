...
(IHconstrs : forall (cs : funsym) (al : list (term * vty))
  ...
  compile get_constructors gen_match gen_let gen_getvars bare simpl_constr (rev al ++ tl) l =
  Some t ->
...
(IHwilds : forall t0 : gen_term b,
  compile get_constructors gen_match gen_let gen_getvars bare simpl_constr tl wilds = Some t0 ->
  exists Hty : gen_typed gamma b t0 ret_ty,
  forall (pd : pi_dom) (pdf : pi_dom_full gamma pd)
...
let comp_wilds := fun _ : unit => compile get_constructors gen_match gen_let gen_getvars bare simpl_constr tl wilds in
...
comp_full tt = Some t1 ->
      exists Hty : gen_typed gamma b t1 ret_ty,
...
Qed.
    ...
    (*NOTE: this method not ideal, need ADT in lots of places
      maybe we want the following:
  ...
  (Hconstrcase: forall t ty tl rl rhd rtl cs params tms,
    ...
    forall (Hsimpl: simplified rl) (Hrl: rl = rhd :: rtl) 
      ...
      (*(IHwilds: forall tm1, P_hyps tl (default rl) -> 
        compile get_constructors gen_match gen_let gen_getvars bare simpl_constr tl wilds = Some tm1 -> 
        P_goal tl (default rl) tm1 Htytl Htywild)*)
      (*Need IH specialized for this case - TODO: remove others I think (and maybe all cslist stuff?)*)
      (IHfun: 
        let new_typs := (map (ty_subst (s_params cs) args) (s_args cs)) in
        forall tm1
...
Proof.
  ...
  - intros. (*full case*)
    ...
    + set (ty := vty_cons (adt_name a) args) in *. 
      ...
      (*Prove equivalence of [bare] cases*)
...
Lemma amap_empty_get_adt
  ...
  ty = vty_cons (adt_name a) args /\ Datatypes.length args = Datatypes.length (m_params m).
    ...
    assert (Hadt: exists m a args, mut_in_ctx m gamma /\ adt_in_mut a m /\
      ty = vty_cons (adt_name a) args /\ length args = length (m_params m)).
    {
      clear -Hisemp Hsimpl Hpop ret_ty Hp gamma_valid. 
      rewrite (amap_not_empty_mem funsym_eq_dec) in Hisemp.
      destruct Hisemp as [c Hintypes].
      (*From [types], know that c is in first column*)
      apply (populate_all_in _ _ _ _ Hsimpl Hpop) in Hintypes.
    ...
    destruct Hadt as [m [a [args [m_in [a_in [Hty args_len]]]]]].
      ...
      eapply Hconstrnotincase; eauto.



      destruct simpl_constr eqn : Hsimplconstr; [|contradiction].
      destruct (is_fun t) as [[[ [cs tys] tms] Ht]|]; [|contradiction].
      simpl in Ht, Hwildcases.
      apply andb_false_iff in Hwildcases.
...
Proof.
          ...
          Some (gen_rep gamma_valid pd pdf pf vt v ret_ty t Hty)).
  {
    ...
    {
      unfold vsymbol in *.
      apply Forall2_app; auto. apply Forall2_rev.
      apply (Hclist_new _ _ _ Hinc).
    }*)
    
      (* Search Forall2 term_has_type.

apply (constr_case_tys (proj1 (Hclist_new c tys2 ps2 Hinc))
        (proj1 (proj2 (Hclist_new c tys2 ps2 Hinc))) Htytl).
      (*TODO: will need this! for proving Hclist_types*)
      (*Prove terms are typed correctly*) 
      unfold vsymbol in *.
      apply Forall2_app.
      2: { inversion Htmty; auto. }
      apply Forall2_rev.
      (*Prove all variables have correct type*)
      apply Forall2_nth; simpl_len; split; [auto|].
      intros i d1 d2 Hi.
      ...
      apply T_Var'.
      -- (*We proved [valid_type] already*)
        specialize (in_cslist_val Hp Hpop Hsimp Hinc) as Hcallval.
        rewrite Forall_forall in Hcallval; apply Hcallval, nth_In. solve_len.
      -- unfold new_vars, new_typs. rewrite combine_nth by solve_len.
          simpl. apply nth_indep; solve_len.
    } *)
   ...
   (* assert (Hp' : pat_matrix_typed (rev new_typs ++ map snd tl) (spec rl c)).
    {
      assert (c_in:= Hclist_in _ _ _ Hinc). 
      assert (Hargs := Hclist_tys _ _ _ Hinc); subst.
      apply (spec_typed_adt m_in a_in); auto.
    }
    (*Now we use IH*)
    exists Htys. exists Hp'.*)
    revert IHconstrs.
...
Qed.
    ...
    (* specialize (IHconstrs _ (combine tms
      (map (ty_subst (s_params cs) args) (s_args cs))) _ Hgetf). 
    ...
    assert (Htysrev: Forall2 (term_has_type gamma)
      (rev tms ++ map fst tl)
      (rev (map (ty_subst (s_params cs) args) (s_args cs)) ++
    map snd tl)).
    {
      apply Forall2_app.
    ...
    specialize (IHconstrs Htysrev Hspecty tm1 Hcomp).
    destruct IHconstrs as [IHty IHmatch].
    exists IHty. intros pd pdf pf vt v Hdisj.
    specialize (IHmatch pd pdf pf vt v (disj_spec Hdisj)).
    (*Now we use spec lemma*)
    specialize (Hsem pd pdf pf vt v).
    rewrite (spec_match_eq pd pdf pf vt v (Tfun cs args tms) m_in a_in f_in args_len constrs_len Htty _
      Hsem _ _ Htmtys rl Hsimp Hp Hspecty).
    (*Now, we use the IH*)
    unfold matches_matrix_tms in IHmatch.
    revert IHmatch.
    unfold matches_matrix_tms.
    match goal with 
    | |- matches_matrix _ _ _ _ _ ?l1 ?al1 ?P ?H = Some ?x ->
         matches_matrix _ _ _ _ _ ?l2 ?al2 ?P2 ?H = Some ?y =>
      replace al1 with al2; auto
    end.
    (*And now, use [al] equality result*)
    ...
    rewrite matches_matrix_irrel with (Hty2:=(default_typed Hp)). auto.
    
    }
    (*case 2: types non empty, matching on constr not in types*)
...
(*Ill-typed (populate_all or dispatch don't give Some)*)
    ...
    + (*Final typing contradiction - dispatch1_opt is None, same as previous.*)
      ...
      discriminate.
    simpl. 
    simpl matches_matrix.

 unfold matches_matrix_tms. simp terms_to_hlist.
    intros tm. inv Hsome.
    destruct ps as [| phd ptl].
 ...
 simpl 
  ...
  - (*The interesting case*)
    ...
    set (comp_wilds := fun (_: unit) => compile get_constructors gen_match gen_let gen_getvars bare simpl_constr tl
      wilds) in *.
    set (comp_cases := fun cs (al : list (term * vty)) =>
      comp_cases (compile get_constructors gen_match gen_let gen_getvars bare simpl_constr) cases tl cs al).
    simpl.
    ...
    (*Now case analysis - we did all [comp_full] cases*)
    destruct simpl_constr; [|solve[auto]].
    destruct (is_fun t) as [s|]; [|solve[auto]].
    destruct s as [[[cs params] tms] Ht]; simpl in Ht |- *.
    destruct (is_constr cs) eqn : Hconstr; [|solve[auto]].
    (*Now get [tm_semantic_constr] info*)
    subst t; simpl.
     (*We know that f is a constructor, so we can get its arg_list*)
    assert (Htty: term_has_type gamma (Tfun cs params tms) (vty_cons (adt_name a) args)). {
    ...
    subst params.
    (*We don't have the valuation v yet, so prove for any*)
    assert (Hsem: forall pd pdf pf vt v,
      tm_semantic_constr pd pdf pf vt v (Tfun cs args tms) 
        m_in a_in f_in args_len Htty 
      (cast_arg_list (eq_sym (sym_sigma_args_map vt cs args constrs_len))
        (terms_to_hlist pd pdf pf vt v tms
        (ty_subst_list (s_params cs) args (s_args cs)) Htms'))).
    {
      intros pd pdf pf vt v.
      (*First, destruct [find_semantic_constr], then use [tfun_semantic_constr]*)
      destruct (find_semantic_constr pd pdf pf vt v (Tfun cs args tms) m_in a_in args_len Htty) as 
        [c [[c_in al] Hrep]]; simpl in Hrep.
      destruct (tfun_semantic_constr _ _ _ _ _ _ _ f_in _ _ _ _ _ constrs_len _ Htms'  Hrep) as [Heq Hal];
      subst. simpl in Hrep. unfold cast_arg_list at 1 in Hrep; simpl in Hrep.
      assert (f_in = c_in) by (apply bool_irrelevance); subst; apply Hrep.
    ...
    destruct (amap_mem funsym_eq_dec cs types) eqn : Hfintypes; auto.
    2: {
      (*If not in types, similar to above, use default result*)
      unfold comp_wilds.
      assert (Hnotin: constr_at_head_ex cs rl = false).
      ...
      intros tm1 Hcomp.
      apply IHwilds in Hcomp.
      destruct Hcomp as [Htyw Hrep]. exists Htyw.
      intros pd pdf pf vt v Hdisj.
      subst ty.
      specialize (Hrep pd pdf pf vt v (Hdisjwild Hdisj)).
      clear IHwilds.
      generalize dependent Htywild.
      rewrite Hwilds; intros Htywild Hrep.
      (*Now we apply default lemma 1*)
      specialize (Hsem pd pdf pf vt v).
      rewrite (default_match_eq pd pdf pf vt v _ m_in a_in f_in args_len constrs_len Htty _ Hsem _ 
        _ Htmtys rl Hsimp Hnotin Hp (default_typed Hp)).
      (*And use IH about wilds*)
      revert Hrep.
      unfold matches_matrix_tms.
    ...
    unfold comp_cases, Pattern.comp_cases.
    assert (Hgetf: amap_get funsym_eq_dec cases cs = Some (spec rl cs)) by
      (unfold cases; rewrite Hdisp; apply (dispatch1_equiv_spec _ _ _ Hsimp Hpop Hp Hfintypes)).
    rewrite Hgetf.
    (*Now can use [spec] lemma*)
    subst ty.
    pose proof (@spec_typed_adt _ _ _ m_in a_in f_in _ (map snd tl) _ Hp) as Hspecty.
...
Proof.
  ...
  - (*Main case*)
    ...
    (union vsymbol_eq_dec (tm_fv t) (big_union vsymbol_eq_dec tm_fv (map fst tl)))
    (pat_mx_tm_fv b rl))).
    {
      unfold comp_full.
    ...
    2: {
      ...
      intros x Hinx. simpl_set_small.
      destruct Hinx as [Hinx | Hinx]; auto.
      right. apply pat_mx_tv_fv_default. rewrite <- Hwilds; assumption.
    }
    (*Constr case*)
    unfold comp_cases.
    destruct (amap_get funsym_eq_dec cases cs) as [y|] eqn : Hget; [|discriminate].
    ...
    rewrite amap_mem_spec in Hmem.
    destruct (amap_get funsym_eq_dec types cs) as [pats|] eqn : Htypes; [|discriminate].
    (*First, prove for other tys, then use typing to show they are the same*)
    assert (Htys: exists tys, In (cs, tys, pats) cslist). {
      apply (proj2 (populate_all_fst_snd_full _ _ _ Hsimpl Hpop)); auto.
    }
    destruct Htys as [tys  Htys].
    unfold cslist in Htys.
    assert (Hconstrargs: existsb (constr_args_at_head_strong cs tys pats) rl).
    { eapply populate_all_snd_strong; eauto. }
    apply existsb_exists in Hconstrargs.
    destruct Hconstrargs as [[rps rt] [Hinrow Hconstrargs]].
    ...
    assert (Hspec: y = spec b rl cs).
    {
      ...
      assert (Hmem1: amap_mem funsym_eq_dec cs (fst types_cslist)).
        rewrite amap_mem_spec, Hget1; auto.
      pose proof (dispatch1_equiv_spec b ret_ty gen_let _ t Hsimpl Hpop Hp Hmem1)
        as Hspec.
      apply dispatch1_opt_some in Hdisp.
      destruct Hdisp as [Hnotnull Hcasewild]; subst.
      unfold cases, types in Hget.
    ...
    subst y.
    assert (Htyt: term_has_type gamma (Tfun cs params tms) (vty_cons (adt_name a) tys)).
    { inversion Htms; subst; auto. }
    (*TODO: do we need to know that tys = params?*)
    assert (Hlentms: length tms = length pats).
    { subst. simpl in Htms. inversion Htyt; subst; lia. }
    (*Now we use the IH*)
    ...
    2: {
      ...
      inversion Htyt; subst.
      subst sigma.
      rewrite (adt_constr_subst_ret gamma_valid m_in a_in cs_in) in H7 by auto.
