...
Lemma replace_tm_rep {gamma: context} 
  ...
  (Hrep: forall (vv: val_vars pd vt) Hty1 Hty2,
    term_rep gamma_valid pd pf vt vv tm_o ty1 Hty1 =
    term_rep gamma_valid pd pf vt vv tm_n ty1 Hty2)
  (t: term) (f: formula) :
  (forall (vv: val_vars pd vt) (ty2: vty)
    (Htyt: term_has_type gamma t ty2)
    (Htysub: term_has_type gamma (replace_tm_t tm_o tm_n t) ty2),
    term_rep gamma_valid pd pf vt vv (replace_tm_t tm_o tm_n t) ty2 Htysub =
    term_rep gamma_valid pd pf vt vv t ty2 Htyt
  ) /\
  (forall (vv: val_vars pd vt) 
    (Hvalf: formula_typed gamma f)
    (Hvalsub: formula_typed gamma (replace_tm_f tm_o tm_n f)),
    formula_rep gamma_valid pd pf vt vv (replace_tm_f tm_o tm_n f) Hvalsub =
...
Proof.
  ...
  - (*Tmatch*)
    ...
    destruct (match_val_single gamma_valid pd pf vt v p1 (Forall_inv Hpatt)
    (term_rep gamma_valid pd pf vt vv tm v Htyt)) eqn : Hmatch; auto.
    destruct (existsb _ _); auto.
  ...
  - (*Fmatch*)  
    ...
    destruct (match_val_single gamma_valid pd pf vt v p1 (Forall_inv Hpatf)
    (term_rep gamma_valid pd pf vt vv tm v Hvalf)) eqn : Hmatch; auto.
    destruct (existsb _ _); auto.
...
Lemma replace_fmla_rep {gamma: context} 
  ...
  (*Need to be the same for all vals, not under a
    particular one*)
  (Hrep: forall (vv: val_vars pd vt) Hty1 Hty2,
    formula_rep gamma_valid pd pf vt vv fmla_o Hty1 =
    formula_rep gamma_valid pd pf vt vv fmla_n Hty2)
  (t: term) (f: formula) :
  (forall (vv: val_vars pd vt) (ty2: vty)
    (Htyt: term_has_type gamma t ty2)
    (Htysub: term_has_type gamma (replace_fmla_t fmla_o fmla_n t) ty2),
    term_rep gamma_valid pd pf vt vv (replace_fmla_t fmla_o fmla_n t) ty2 Htysub =
    term_rep gamma_valid pd pf vt vv t ty2 Htyt
  ) /\
  (forall (vv: val_vars pd vt) 
    (Hvalf: formula_typed gamma f)
    (Hvalsub: formula_typed gamma (replace_fmla_f fmla_o fmla_n f)),
    formula_rep gamma_valid pd pf vt vv (replace_fmla_f fmla_o fmla_n f) Hvalsub =
    formula_rep gamma_valid pd pf vt vv f Hvalf).
Proof.
  revert t f; apply term_formula_ind; simpl; intros;
