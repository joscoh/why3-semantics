...
let print_option_or_default default f fmt = function
  ...
  | Some x -> f fmt x
   ...
   (*JOSH - why do we need to add ignore when ocaml doesnt?*)
let rec print_list_pre sep print fmt = function
  | [] -> ()
  | x :: r -> ignore(sep fmt ()); print fmt x; print_list_pre sep print fmt r

 
let rec print_list_suf (sep : 'a -> unit -> 'b) print fmt = function
  | [] -> ()
  | x :: r -> print fmt x; sep fmt (); print_list_suf sep print fmt r

let print_list (sep : 'a -> unit -> 'b) print fmt = function
  | [] -> ()
  | [x] -> print fmt x
